...
<div class="h-[77%] flex flex-col">
    ...
    <textarea name="description"  rows="4" placeholder="Enter description"
        class="flex-1 w-full px-4 py-2 border @error('description') border-red-500 @else border-gray-300 @enderror rounded-lg focus:ring-2 focus:ring-emerald-400 focus:outline-none bg-white resize-none">{{ $product->description }}</textarea>
    @error('description')
        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
    @enderror
</div>
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <input type="number" name="stock" value="{{$product->stock}}" placeholder="Enter stock"
    ...
    <!-- if checked, this 1 overwrites the 0 -->
    <input
        type="checkbox"
        name="is_active"
        id="is_active"
